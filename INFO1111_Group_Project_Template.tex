...
\subsection{Facebook, LAMP: April Wang}

Facebook is an American online social media and social networking service, providing services to users with devices with internet connectivity. Users can create their personal profile including text, photos and videos after registering. 

LAMP stands for Linux, Apache, MySQL and PHP, this tech stack provide a proven set of software for delivering high-performance web applications. Due to the maturity and the wide-spread popularity of the LAMP tech stack, its high compatibility and maximised uptimes makes it suitable for Facebook. 

The LAMP tech stack uses Linux for the operating system (OS), which is a free and open source OS that consists of a packaging management system around Linux kernel. Linux offers more flexibility and configuration options than some other operating systems. 

Apache is the web server that processes requests and serves up web assets via HTTP so that the application is accessible to anyone in the public domain over a simple web URL. It is Since Apache’s mature, feature-rich server that runs a large share of the websites currently on the internet. 

Within the LAMP tech stack, they use My SQL as a relational database, which is an open source relational database(ibm) management system for storing application data. MySQL has wide spread popularity and is supported by most host providers. At Facebook, MySQL is used to manage many petabytes of data, along with the InnoDB storage engine that serves social activities like comments, likes and shares. In addition, MySQL is amenable to automation, making it easy for a small team to manage thousands of MySQL servers while providing high quality service (Matsunobu, 2016), which perfectly suits the core function of Facebook as an online social platform. 

PHP is the programming language in the recent LAMP tech stack with extensive support and active developer community. It is an open source scripting language words with Apache to help creating dynamic web pages. All these features serves are suitable in the making up of the core infrastructures in Facebook’s front end server. 
